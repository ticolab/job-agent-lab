...
- :class:`RunContext` — a frozen bundle of the per-run knobs the CLI
  ...
  agent step cap, and the :class:`~job_agent_lab.domain.region.TargetRegion`
...
- :data:`STRATEGIES` + :func:`get_strategy` — the registry the CLI
  ...
  :mod:`job_agent_lab.extraction`'s ``__init__.py`` at package-load
  time. ``get_strategy`` raises ``ValueError`` for unknown names (belt
  behind the ``StrategyName`` ``Literal`` on
  :class:`~job_agent_lab.domain.company.Company`).

:func:`build_report` is the *single* author of the on-disk / print
report shape. Both :class:`~job_agent_lab.extraction.dom.strategy.DomStrategy`
...
class RunContext:
    ...
    Attributes:
        ...
        region: The :class:`TargetRegion` the run is scoped to. The DOM
            ...
            (via
            :func:`job_agent_lab.navigation.prompt.build_goal_prompt`);
...
def build_report(
    *,
    ...
    :func:`job_agent_lab.reporting.output.save_result` writes and the
    input :func:`~job_agent_lab.reporting.output.print_summary`
...
# Populated by :mod:`job_agent_lab.extraction`'s package ``__init__``
