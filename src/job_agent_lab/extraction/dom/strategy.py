...
wrapped in the :class:`~job_agent_lab.extraction.base.ExtractionStrategy`
protocol so the CLI can dispatch through
:func:`~job_agent_lab.extraction.base.get_strategy` uniformly with the
API strategies (:class:`~job_agent_lab.extraction.ats.greenhouse.GreenhouseStrategy`
and future siblings).

The heavy lifting is unchanged from pre-SYS-4:

- :func:`~job_agent_lab.navigation.controller.build_controller` — the two
  custom tools the agent uses (``extract_job_links``,
  ``report_no_matching_location_filter``).
- :func:`~job_agent_lab.navigation.runner.build_agent` — the
  ``Agent`` + ``BrowserSession`` wiring (LLM, browser profile, task
  prompt).
- :class:`~job_agent_lab.domain.results.ExtractionResult` — pydantic
...
:func:`~job_agent_lab.extraction.base.build_report`. ``_build_output``
...
from job_agent_lab.domain.results import ExtractionResult
...
class DomStrategy:
    ...
    async def extract(self, company: Company, ctx: RunContext) -> dict[str, Any]:
        ...
        :class:`~job_agent_lab.domain.region.TargetRegion` on ``ctx`` is
        consumed indirectly: the agent's ``GOAL_PROMPT`` and Case C
        tool description were already rendered from
        :data:`~job_agent_lab.domain.region.COSTA_RICA_LATAM` at import
        ...
        an inert :class:`~job_agent_lab.domain.company.RuntimeHooks`
    ...
    async def _extract_prefiltered(
        ...
        :func:`~job_agent_lab.navigation.runner.build_agent` is never
        ...
        :mod:`job_agent_lab.settings` for exactly this parity). Each
        ...
        :class:`~job_agent_lab.domain.company.Company` does not
    ...
    *,
    ...
    Args:
        ...
        path_prefix: Optional explicit override for the same-origin path
            ...
            ``navigation.controller.build_controller`` for details.
        ...
        expected_jobs: SYS-9 human-counted target (mirrors
            ...
            :func:`~job_agent_lab.extraction.base.build_report` so the
        ...
        hooks: SYS-12 :class:`~job_agent_lab.domain.company.RuntimeHooks`
    ...
    Returns:
        ...
        authored by
        :func:`~job_agent_lab.extraction.base.build_report`).
    """
    # Deferred imports: ``navigation.controller`` transitively loads
    ...
    # any caller that imports ``navigation.controller`` first (e.g. the
    ...
    from job_agent_lab.navigation.runner import build_agent
...
def _build_output(
    *,
    ...
    Delegates to :func:`~job_agent_lab.extraction.base.build_report` for
    ...
    :func:`~job_agent_lab.extraction.base.compute_verdict` — this
