...
- ``filter_tokens`` — the UI-visible options the agent may click in a
  ...
  in :mod:`job_agent_lab.navigation.prompt` and into the Case C tool
  description in :mod:`job_agent_lab.navigation.controller`. The first
...
- ``matches(location_name)`` — a predicate the API-based adapters
  (:mod:`job_agent_lab.extraction.ats.greenhouse` and future siblings)
...
class TargetRegion:
    ...
    def format_quoted_options(self) -> str:
        ...
        :mod:`job_agent_lab.navigation.prompt`.
        """
    ...
    def format_unquoted_options(self) -> str:
        ...
        :mod:`job_agent_lab.navigation.controller`.
        """
