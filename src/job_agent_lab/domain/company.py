...
:mod:`job_agent_lab.extraction`; a unit test asserts the two stay in
sync.
...
# implementation in :mod:`job_agent_lab.extraction.STRATEGIES` must map
...
class PhenomConfig(BaseModel):
    ...
    (:func:`~job_agent_lab.extraction.ats.phenom.build_request_body`),
...
class TalentbrewConfig(BaseModel):
    ...
    (:func:`~job_agent_lab.extraction.ats.talentbrew.build_query_params`),
...
class CoveoConfig(BaseModel):
    ...
    (:func:`~job_agent_lab.extraction.ats.coveo.build_search_body`).
...
class Company(BaseModel):
    ...
    Attributes:
        ...
        strategy: Which extraction strategy handles this company. The
            ...
            :meth:`~job_agent_lab.domain.region.TargetRegion.matches`.
    ...
    def _validate_pre_filter_urls_require_dom_strategy(self) -> Self:
        ...
        (:class:`~job_agent_lab.extraction.dom.strategy.DomStrategy`);
        the Greenhouse API strategy hits the boards API directly and
