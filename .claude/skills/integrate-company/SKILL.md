...
begin with `mcp__playwright__` (e.g. `mcp__playwright__browser_navigate`);
recipes use the shorthand form.
...
9. **Commit the integration.** Stage **only** the `catalog/companies.py` edit
   ...
   `Co-Authored-By:` trailer naming the model actually running the session
   (e.g. `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`) — do not
   hardcode a model version, it changes between sessions. The
...
- **Commit exactly once, only on success, never push, never branch.** When all
  ...
  `Co-Authored-By:` trailer naming the running model. Do not stage
...
7. A single `feat: add <Name> to company test sample` commit has been
   ...
   `Co-Authored-By:` trailer naming the running model; all
